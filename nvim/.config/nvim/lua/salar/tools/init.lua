...
function M.setup()
	...
	vim.api.nvim_create_user_command("TypstPreview", function()
		...
		local pdf = file:gsub("%.typ$", ".pdf")
		local opener = vim.fn.executable("zathura") == 1 and "zathura"
			or (vim.fn.executable("open") == 1 and "open" or nil)

		if opener then
			vim.fn.jobstart({ opener, pdf }, { detach = true })
		else
			vim.notify("No PDF opener found for " .. pdf, vim.log.levels.WARN)
		end
	end, {})
