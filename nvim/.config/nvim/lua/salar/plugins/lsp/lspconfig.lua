		...
		vim.lsp.config("lua_ls", {
			...
			vim.api.nvim_create_autocmd("BufWritePre", {
				callback = function(ev)
					...
					local go_like = vim.tbl_contains({ "go", "gomod", "gowork", "gotmpl" }, ft)
					...
					if not cpp_like and not go_like then
						return
					end
					...
					vim.lsp.buf.format({
						...
						filter = function(client)
							return (cpp_like and client.name == "clangd") or (go_like and client.name == "gopls")
						end,
					})
				end,
			})
